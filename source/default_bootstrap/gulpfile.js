...
gulp.task('html', gulp.series(function(){
    return gulp.src(['src/html/**/*.html','!src/html/**/_*.html'], { sourcemaps: true })
        .pipe(rig())
        .pipe(gulp.dest('dist/html', { sourcemaps: './maps' }))
...
gulp.task('js', function (){
    return gulp.src(['src/js/scripts.js','src/js/libs/jquery.js', '!src/js/**/_*.js'], { sourcemaps: true })
        .pipe(rig())
        .pipe(uglify())
        .pipe(rename({
            suffix: '.min'
...
//         .pipe(gulp.dest("dist/fonts", { sourcemaps: './maps' }))
//         .pipe(browserSync.stream());
// });
...
gulp.task('serve', gulp.series('html', 'sass', 'js', 'img', function() {
    ...
    browserSync.init({
        server: "./dist",
        startPath: "/html/views/home.html"
    });
